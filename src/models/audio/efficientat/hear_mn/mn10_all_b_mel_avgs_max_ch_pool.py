...
from .models import feature_pooling
...
def load_model(
    ...
    feature_pooling_fn=feature_pooling.max_ch_pool,
):
    model = get_basic_model(
        model_name=model_name, mode=mode, feature_pooling_fn=feature_pooling_fn
    )
...
def get_basic_model(
    model_name="mn10_as", feature_pooling_fn=feature_pooling.max_ch_pool, **kwargs
):
    ...
    model = MNHearWrapper(
        mel=mel,
        net=net,
        scene_embedding_size=600,
        timestamp_embedding_size=600,
        **kwargs
    )
    return model
