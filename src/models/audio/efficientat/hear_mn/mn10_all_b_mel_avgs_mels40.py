...
def get_basic_model(model_name="mn10_as_mels_40", **kwargs):
    mel = AugmentMelSTFT(n_mels=40, sr=32000, win_length=800, hopsize=320, n_fft=1024)
    net = get_model(width_mult=NAME_TO_WIDTH(model_name), pretrained_name=model_name)
    model = MNHearWrapper(
        mel=mel,
        net=net,
        scene_embedding_size=512,
        timestamp_embedding_size=512,
        **kwargs
    )
    return model
