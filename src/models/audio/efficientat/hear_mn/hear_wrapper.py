...
class MNHearWrapper(nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad(), autocast(device_type=self.device().type) if self.device().type.startswith(
            "cuda"
        ) else nullcontext():
            specs = self.mel(x)
    ...
    def get_timestamp_embeddings(self, audio: torch.Tensor, window_size=None, hop=None, pad=None):
        ...
        if pad is None:
            pad = window_size // 2
        # audio = audio.cpu() # FIXME: don't move to CPU
        n_sounds, n_samples = audio.shape
        ...
        segments = (
            F.unfold(padded, kernel_size=(1, window_size), stride=(1, hop)).transpose(-1, -2).transpose(0, 1)
        )
        timestamps = []
        embeddings = []
        for i, segment in enumerate(segments):
            timestamps.append(i)
            # FIXME: don't move embeddings to CPU since needs to be transferred back to GPU during training
            # embeddings.append(self.forward(segment.to(self.device())).cpu())
            embeddings.append(self.forward(segment.to(self.device())))
