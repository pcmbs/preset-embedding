...
of the form `audio_embeddings_<start>_<end>.pkl` and `synth_params_<start>_<end>.pkl`,
...
under the name `audio_embeddings.pkl` and `synth_params.pkl`.

A AssertionError will be raised if:
- the subsets do not cover the entire dataset (checked both for audio_embeddings and synth_params)
...
def cat_dataset_pkl(cfg: DictConfig) -> None:
    ...
    for pkl_file in sorted(Path.cwd().glob("*.pkl")):
        ...
        elif pkl_file.stem.startswith("synth_params_"):
            pkl_filename["synth_params"].append(
                {
                    "filename": pkl_file.name,
                    "start": int(pkl_file.stem.split("_")[2]),
                    "end": int(pkl_file.stem.split("_")[3]),
    ...
    if not check_concatenated_slices_coverage(pkl_filename["synth_params"], dataset_size):
    ...
    synth_params = torch.cat(
        [torch.load(Path.cwd() / pkl_file["filename"]) for pkl_file in pkl_filename["synth_params"]]
    )
    ...
    if synth_params.shape[1] != in_features:
        raise AssertionError(
            f"Number of input features (synth params) does not match {in_features}, got {synth_params.shape[1]}."
        )

    log.info(f"\nSuccessfully concatenated dataset with shape: {audio_emb.shape=} and {synth_params.shape=}")

    log.info(f"Saving audio embeddings to {Path.cwd() / 'audio_embeddings.pkl'}...")
    torch.save(audio_emb, Path.cwd() / "audio_embeddings.pkl")

    log.info(f"Saving audio embeddings to {Path.cwd() / 'synth_params.pkl'}...")
    torch.save(synth_params, Path.cwd() / "synth_params.pkl")

    if cfg.delete_subsets:
        log.info("Deleting original subsets...")
        for file_info in pkl_filename["audio_emb"] + pkl_filename["synth_params"]:
